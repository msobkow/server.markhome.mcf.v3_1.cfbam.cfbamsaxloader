...
// Description: Java 25 XML SAX Element Handler for TableTweak
...
/*
 *	CFBamSaxLoaderTableTweakParse XML SAX Element Handler implementation
 *	for TableTweak.
 */
public class CFBamSaxLoaderTableTweak
	extends CFLibXmlCoreElementHandler
{
	public CFBamSaxLoaderTableTweak( CFBamSaxLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		...
		ICFBamTableTweakObj origBuff = null;
		ICFBamTableTweakEditObj editBuff = null;
		...
		// TableTweak Attributes
		// TableTweak References
		ICFBamTableObj refTableDef = null;
		...
		try {
			assert qName.equals( "TableTweak" );
			...
			origBuff = (ICFBamTableTweakObj)schemaObj.getTableTweakTableObj().newInstance();
			editBuff = (ICFBamTableTweakEditObj)origBuff.beginEdit();
			...
			if( scopeObj == null ) {
				...
				refTableDef = (ICFBamTableObj) scopeObj;
				editBuff.setRequiredContainerTableDef( refTableDef );
			}
			else {
			...
			CFBamSaxLoader.LoaderBehaviourEnum loaderBehaviour = saxLoader.getTableTweakLoaderBehaviour();
			ICFBamTableTweakEditObj editTableTweak = null;
			ICFBamTableTweakObj origTableTweak = (ICFBamTableTweakObj)schemaObj.getTableTweakTableObj().readTableTweakByUNameIdx( refScopeDef.getRequiredId(),
			editBuff.getRequiredName() );
			if( origTableTweak == null ) {
				editTableTweak = editBuff;
			}
			else {
				switch( loaderBehaviour ) {
					case Insert:
						break;
					case Update:
						editTableTweak = (ICFBamTableTweakEditObj)origTableTweak.beginEdit();
						editTableTweak.setRequiredTenantId( editBuff.getRequiredTenantId() );
						editTableTweak.setOptionalDefSchemaTenantId( editBuff.getOptionalDefSchemaTenantId() );
						editTableTweak.setRequiredName( editBuff.getRequiredName() );
						editTableTweak.setRequiredReplacesInherited( editBuff.getRequiredReplacesInherited() );
						editTableTweak.setRequiredTweakGelText( editBuff.getRequiredTweakGelText() );
						editTableTweak.setOptionalLookupDefSchema( editBuff.getOptionalLookupDefSchema() );
						break;
					case Replace:
						editTableTweak = (ICFBamTableTweakEditObj)origTableTweak.beginEdit();
						editTableTweak.deleteInstance();
						editTableTweak = null;
						origTableTweak = null;
						editTableTweak = editBuff;
						break;
			...
			if( editTableTweak != null ) {
				if( origTableTweak != null ) {
					editTableTweak.update();
				}
				else {
					origTableTweak = (ICFBamTableTweakObj)editTableTweak.create();
				}
				editTableTweak = null;
			}

			curContext.putNamedValue( "Object", origTableTweak );
		}
		catch( RuntimeException e ) {
