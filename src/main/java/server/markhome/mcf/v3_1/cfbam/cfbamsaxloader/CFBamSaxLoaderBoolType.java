...
// Description: Java 25 XML SAX Element Handler for BoolType
	...
	extends CFLibXmlCoreElementHandler
{
	...
	throws SAXException
	{
		...
		ICFBamBoolTypeObj origBuff = null;
		ICFBamBoolTypeEditObj editBuff = null;
		...
		try {
			...
			origBuff = (ICFBamBoolTypeObj)schemaObj.getBoolTypeTableObj().newInstance();
			editBuff = (ICFBamBoolTypeEditObj)origBuff.beginEdit();
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				throw new CFLibUsageException( getClass(),
					S_ProcName,
					String.format(Inz.x("cflib.xml.CFLibXmlUtil.XmlBooleanInvalid"), "InitValue", attrInitValue),
					String.format(Inz.s("cflib.xml.CFLibXmlUtil.XmlBooleanInvalid"), "InitValue", attrInitValue));
			}
			...
			ICFBamBoolTypeObj origBoolType = (ICFBamBoolTypeObj)schemaObj.getBoolTypeTableObj().readBoolTypeByUNameIdx( refScope.getRequiredId(),
			...
			if( origBoolType == null ) {
				...
				switch( loaderBehaviour ) {
					...
					case Update:
						...
						editBoolType.setRequiredImplementsPolymorph( editBuff.getRequiredImplementsPolymorph() );
