...
// Description: Java 25 XML SAX Element Handler for NumberType
	...
	extends CFLibXmlCoreElementHandler
{
	...
	throws SAXException
	{
		...
		ICFBamNumberTypeObj origBuff = null;
		ICFBamNumberTypeEditObj editBuff = null;
		...
		try {
			...
			origBuff = (ICFBamNumberTypeObj)schemaObj.getNumberTypeTableObj().newInstance();
			editBuff = (ICFBamNumberTypeEditObj)origBuff.beginEdit();
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				natInitValue= CFLibBigDecimalUtil.parse( "CFBam.NumberDef.InitValue", "CFBam.NumberDef.InitValue", 31, 5, attrInitValue );
			}
			...
			if( ( attrMinValue == null ) || ( attrMinValue.length() <= 0 ) ) {
				...
				natMinValue= CFLibBigDecimalUtil.parse( "CFBam.NumberDef.MinValue", "CFBam.NumberDef.MinValue", 31, 5, attrMinValue );
			}
			...
			if( ( attrMaxValue == null ) || ( attrMaxValue.length() <= 0 ) ) {
				...
				natMaxValue= CFLibBigDecimalUtil.parse( "CFBam.NumberDef.MaxValue", "CFBam.NumberDef.MaxValue", 31, 5, attrMaxValue );
			}
			...
			ICFBamNumberTypeObj origNumberType = (ICFBamNumberTypeObj)schemaObj.getNumberTypeTableObj().readNumberTypeByUNameIdx( refScope.getRequiredId(),
			...
			if( origNumberType == null ) {
				...
				switch( loaderBehaviour ) {
					...
					case Update:
						...
						editNumberType.setRequiredImplementsPolymorph( editBuff.getRequiredImplementsPolymorph() );
