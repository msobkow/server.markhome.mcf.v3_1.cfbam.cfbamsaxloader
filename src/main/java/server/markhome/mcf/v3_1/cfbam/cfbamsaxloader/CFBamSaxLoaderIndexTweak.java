...
// Description: Java 25 XML SAX Element Handler for IndexTweak
...
/*
 *	CFBamSaxLoaderIndexTweakParse XML SAX Element Handler implementation
 *	for IndexTweak.
 */
public class CFBamSaxLoaderIndexTweak
	extends CFLibXmlCoreElementHandler
{
	public CFBamSaxLoaderIndexTweak( CFBamSaxLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		...
		ICFBamIndexTweakObj origBuff = null;
		ICFBamIndexTweakEditObj editBuff = null;
		...
		// IndexTweak Attributes
		// IndexTweak References
		ICFBamIndexObj refIndexDef = null;
		...
		try {
			assert qName.equals( "IndexTweak" );
			...
			origBuff = (ICFBamIndexTweakObj)schemaObj.getIndexTweakTableObj().newInstance();
			editBuff = (ICFBamIndexTweakEditObj)origBuff.beginEdit();
			...
			if( scopeObj == null ) {
				...
				refIndexDef = (ICFBamIndexObj) scopeObj;
				editBuff.setRequiredContainerIndexDef( refIndexDef );
			}
			else {
			...
			CFBamSaxLoader.LoaderBehaviourEnum loaderBehaviour = saxLoader.getIndexTweakLoaderBehaviour();
			ICFBamIndexTweakEditObj editIndexTweak = null;
			ICFBamIndexTweakObj origIndexTweak = (ICFBamIndexTweakObj)schemaObj.getIndexTweakTableObj().readIndexTweakByUNameIdx( refScopeDef.getRequiredId(),
			editBuff.getRequiredName() );
			if( origIndexTweak == null ) {
				editIndexTweak = editBuff;
			}
			else {
				switch( loaderBehaviour ) {
					case Insert:
						break;
					case Update:
						editIndexTweak = (ICFBamIndexTweakEditObj)origIndexTweak.beginEdit();
						editIndexTweak.setRequiredTenantId( editBuff.getRequiredTenantId() );
						editIndexTweak.setOptionalDefSchemaTenantId( editBuff.getOptionalDefSchemaTenantId() );
						editIndexTweak.setRequiredName( editBuff.getRequiredName() );
						editIndexTweak.setRequiredReplacesInherited( editBuff.getRequiredReplacesInherited() );
						editIndexTweak.setRequiredTweakGelText( editBuff.getRequiredTweakGelText() );
						editIndexTweak.setOptionalLookupDefSchema( editBuff.getOptionalLookupDefSchema() );
						break;
					case Replace:
						editIndexTweak = (ICFBamIndexTweakEditObj)origIndexTweak.beginEdit();
						editIndexTweak.deleteInstance();
						editIndexTweak = null;
						origIndexTweak = null;
						editIndexTweak = editBuff;
						break;
			...
			if( editIndexTweak != null ) {
				if( origIndexTweak != null ) {
					editIndexTweak.update();
				}
				else {
					origIndexTweak = (ICFBamIndexTweakObj)editIndexTweak.create();
				}
				editIndexTweak = null;
			}

			curContext.putNamedValue( "Object", origIndexTweak );
		}
		catch( RuntimeException e ) {
