...
// Description: Java 25 XML SAX Element Handler for UInt64Type
	...
	extends CFLibXmlCoreElementHandler
{
	...
	throws SAXException
	{
		...
		ICFBamUInt64TypeObj origBuff = null;
		ICFBamUInt64TypeEditObj editBuff = null;
		...
		try {
			...
			origBuff = (ICFBamUInt64TypeObj)schemaObj.getUInt64TypeTableObj().newInstance();
			editBuff = (ICFBamUInt64TypeEditObj)origBuff.beginEdit();
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				natInitValue= CFLibBigDecimalUtil.parse( "CFBam.UInt64Def.InitValue", "CFBam.UInt64Def.InitValue", 19, 0, attrInitValue );
			}
			...
			if( ( attrMinValue == null ) || ( attrMinValue.length() <= 0 ) ) {
				...
				natMinValue= CFLibBigDecimalUtil.parse( "CFBam.UInt64Def.MinValue", "CFBam.UInt64Def.MinValue", 19, 0, attrMinValue );
			}
			...
			if( ( attrMaxValue == null ) || ( attrMaxValue.length() <= 0 ) ) {
				...
				natMaxValue= CFLibBigDecimalUtil.parse( "CFBam.UInt64Def.MaxValue", "CFBam.UInt64Def.MaxValue", 19, 0, attrMaxValue );
			}
			...
			ICFBamUInt64TypeObj origUInt64Type = (ICFBamUInt64TypeObj)schemaObj.getUInt64TypeTableObj().readUInt64TypeByUNameIdx( refScope.getRequiredId(),
			...
			if( origUInt64Type == null ) {
				...
				switch( loaderBehaviour ) {
					...
					case Update:
						...
						editUInt64Type.setRequiredImplementsPolymorph( editBuff.getRequiredImplementsPolymorph() );
