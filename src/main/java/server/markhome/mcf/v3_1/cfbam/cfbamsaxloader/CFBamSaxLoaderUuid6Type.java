...
// Description: Java 25 XML SAX Element Handler for Uuid6Type
...
/*
 *	CFBamSaxLoaderUuid6TypeParse XML SAX Element Handler implementation
 *	for Uuid6Type.
 */
public class CFBamSaxLoaderUuid6Type
	extends CFLibXmlCoreElementHandler
{
	public CFBamSaxLoaderUuid6Type( CFBamSaxLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		...
		ICFBamUuid6TypeObj origBuff = null;
		ICFBamUuid6TypeEditObj editBuff = null;
		...
		try {
			...
			origBuff = (ICFBamUuid6TypeObj)schemaObj.getUuid6TypeTableObj().newInstance();
			editBuff = (ICFBamUuid6TypeEditObj)origBuff.beginEdit();
			...
			CFBamSaxLoader.LoaderBehaviourEnum loaderBehaviour = saxLoader.getUuid6TypeLoaderBehaviour();
			ICFBamUuid6TypeEditObj editUuid6Type = null;
			ICFBamUuid6TypeObj origUuid6Type = (ICFBamUuid6TypeObj)schemaObj.getUuid6TypeTableObj().readUuid6TypeByUNameIdx( refScope.getRequiredId(),
			editBuff.getRequiredName() );
			if( origUuid6Type == null ) {
				editUuid6Type = editBuff;
			}
			else {
				switch( loaderBehaviour ) {
					case Insert:
						break;
					case Update:
						editUuid6Type = (ICFBamUuid6TypeEditObj)origUuid6Type.beginEdit();
						editUuid6Type.setRequiredName( editBuff.getRequiredName() );
						editUuid6Type.setOptionalShortName( editBuff.getOptionalShortName() );
						editUuid6Type.setOptionalLabel( editBuff.getOptionalLabel() );
						editUuid6Type.setOptionalShortDescription( editBuff.getOptionalShortDescription() );
						editUuid6Type.setOptionalDescription( editBuff.getOptionalDescription() );
						editUuid6Type.setOptionalDefaultXmlValue( editBuff.getOptionalDefaultXmlValue() );
						editUuid6Type.setRequiredIsNullable( editBuff.getRequiredIsNullable() );
						editUuid6Type.setOptionalGenerateId( editBuff.getOptionalGenerateId() );
						editUuid6Type.setRequiredImplementsPolymorph( editBuff.getRequiredImplementsPolymorph() );
						editUuid6Type.setOptionalDbName( editBuff.getOptionalDbName() );
						editUuid6Type.setOptionalInitValue( editBuff.getOptionalInitValue() );
						editUuid6Type.setOptionalLookupDefSchema( editBuff.getOptionalLookupDefSchema() );
						break;
					case Replace:
						editUuid6Type = (ICFBamUuid6TypeEditObj)origUuid6Type.beginEdit();
						editUuid6Type.deleteInstance();
						editUuid6Type = null;
						origUuid6Type = null;
						editUuid6Type = editBuff;
						break;
			...
			if( editUuid6Type != null ) {
				if( origUuid6Type != null ) {
					editUuid6Type.update();
				}
				else {
					origUuid6Type = (ICFBamUuid6TypeObj)editUuid6Type.create();
				}
				editUuid6Type = null;
			}

			curContext.putNamedValue( "Object", origUuid6Type );
		}
