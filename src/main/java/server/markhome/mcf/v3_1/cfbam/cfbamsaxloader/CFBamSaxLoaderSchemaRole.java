...
// Description: Java 25 XML SAX Element Handler for SchemaRole
...
/*
 *	CFBamSaxLoaderSchemaRoleParse XML SAX Element Handler implementation
 *	for SchemaRole.
 */
public class CFBamSaxLoaderSchemaRole
	extends CFLibXmlCoreElementHandler
{
	public CFBamSaxLoaderSchemaRole( CFBamSaxLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		...
		ICFBamSchemaRoleObj origBuff = null;
		ICFBamSchemaRoleEditObj editBuff = null;
		...
		try {
			assert qName.equals( "SchemaRole" );
			...
			origBuff = (ICFBamSchemaRoleObj)schemaObj.getSchemaRoleTableObj().newInstance();
			editBuff = (ICFBamSchemaRoleEditObj)origBuff.beginEdit();
			...
			CFBamSaxLoader.LoaderBehaviourEnum loaderBehaviour = saxLoader.getSchemaRoleLoaderBehaviour();
			ICFBamSchemaRoleEditObj editSchemaRole = null;
			ICFBamSchemaRoleObj origSchemaRole = (ICFBamSchemaRoleObj)schemaObj.getSchemaRoleTableObj().readSchemaRoleByUNameIdx( refScopeDef.getRequiredId(),
			editBuff.getRequiredName() );
			if( origSchemaRole == null ) {
				editSchemaRole = editBuff;
			}
			else {
			...
			if( editSchemaRole != null ) {
				if( origSchemaRole != null ) {
					editSchemaRole.update();
				}
				else {
					origSchemaRole = (ICFBamSchemaRoleObj)editSchemaRole.create();
				}
				editSchemaRole = null;
			}

			curContext.putNamedValue( "Object", origSchemaRole );
		}
